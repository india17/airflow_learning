...
DESTINATION_BUCKET = "tredence-airflow-bucket"
...
def customer_rating_aggregated_view(**kwargs):
    try:
        ...
        if (
            ...
            logger.info(
                "No new data for customer_rating or customer_information. No analysis performed."
            )
            return
        ...
        output_key = f"bharat/customer_rating_analyses/customer_rating_analyses_{current_date.strftime('%Y-%m-%d_%H-%M-%S')}.csv"
        s3_hook.load_string(
...
with DAG(
    dag_id="bharat_etl_dag_hooks_and_xcoms_and_sensors",
    start_date=days_ago(1),
    owner_links={"airflow": "https://airflow.apache.org/"},
    tags=["hooks", "xcoms", "sensors","bharat"],
    ...
    check_customer_rating_file = S3KeySensor(
        task_id="check_customer_rating_file",
        bucket_name=DESTINATION_BUCKET,
        bucket_key="{{ task_instance.xcom_pull(key='s3_data_path', task_ids='get_last_run_date_group.get_customer_rating_last_run_date') }}",
    ...
    check_customer_information_file = S3KeySensor(
        task_id="check_customer_information_file",
        bucket_name=DESTINATION_BUCKET,
        bucket_key="{{ task_instance.xcom_pull(key='s3_data_path', task_ids='get_last_run_date_group.get_customer_information_last_run_date') }}",
        aws_conn_id="aws_default",
        timeout=5 * 60,
        poke_interval=30,
        mode="poke",
    ...
    (
        get_last_run_date_group
        >> [check_customer_rating_file, check_customer_information_file]
        >> customer_rating_aggregated_view_task
    )
